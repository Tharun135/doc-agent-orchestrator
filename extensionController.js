...
exports.handleGenerateDoc = handleGenerateDoc;
const vscode = __importStar(require("vscode"));
const questionDetector_1 = require("./questionDetector");
const promptBuilder_1 = require("./promptBuilder");
const ambiguityParser_1 = require("./ambiguityParser");
// ─────────────────────────────────────────────────────────────────────────────
// Main command handler — triggered when user selects text and runs the command
// ─────────────────────────────────────────────────────────────────────────────
async function handleGenerateDoc(selectedText, taskType, userIntent, templateContent, callAI) {
    ...
    try {
        ...
        const questions = (0, questionDetector_1.detectQuestions)(selectedText, taskType, templateContent, userIntent);
        ...
        if (questions.length > 0) {
            ...
            preClarifications = (0, questionDetector_1.formatPreClarifications)(questions, answers);
        }
        ...
        (0, promptBuilder_1.validatePromptInput)(input);
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Generating document…", cancellable: false }, async () => {
            const prompt = (0, promptBuilder_1.generatePrompt)(input);
            const generatedDoc = await aiCall(prompt);
            // ── PASS N: Resolve Preserved Ambiguities (optional loop) ────────────
            const finalDoc = await resolveAmbiguitiesIfNeeded(generatedDoc, input, aiCall);
 ...
 * the user declines.
 */
async function resolveAmbiguitiesIfNeeded(doc, baseInput, aiCall, maxPasses = 3) {
    let currentDoc = doc;
    let pass = 2;
    while (pass <= maxPasses + 1) {
        const ambiguities = (0, ambiguityParser_1.parsePreservedAmbiguities)(currentDoc);
        if (ambiguities.length === 0) {
            break;
        }
        const resolve = await vscode.window.showInformationMessage(`${ambiguities.length} ambiguity/ies remain. Would you like to resolve them?`, "Yes", "No");
        if (resolve !== "Yes") {
            break;
        }
        ...
        const clarifications = (0, ambiguityParser_1.formatClarifications)(ambiguities, answers);
        const nextInput = {
            ...baseInput,
            clarifications,
            pass,
        };
        const prompt = (0, promptBuilder_1.generatePrompt)(nextInput);
 ...
 */
async function showQAPanel(questions) {
    return new Promise((resolve) => {
        const panel = vscode.window.createWebviewPanel("docGenQA", `Answer ${questions.length} question${questions.length !== 1 ? "s" : ""} before generating`, vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
function buildQAHtml(questions) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function defaultAICall(prompt) {
